...
namespace velodyne_rawdata
{
  ...
  void RawData::unpack(const velodyne_msgs::VelodynePacket &pkt,
                       VPointCloud &pc,
                            velodyne_msgs::custompoint *my_msg)
//                       velodyne_msgs::custompoint &custompoints) ///내가 만
    {
        ...
        {
            unpack_vlp16(pkt, pc, my_msg);
            return;
        }

        const raw_packet_t *raw = (const raw_packet_t *) &pkt.data[0];

        for (int i = 0; i < BLOCKS_PER_PACKET; i++) {///block의 시작.  i가 한마디로 바뀌는 것
            //blocks_perpacket은 12로 상수 화 해놓은것 같음.
...
//            std::cout << "1111111111111" << std::endl;
            int line_change =(i%2 + 1)*10;
            for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {///여기도 그 한바퀴 도는 순간 같은데.
...
//                }
                ...
                float x, y, z;
                float intensity;
                uint8_t laser_number;       ///< hardware laser number //뭐여?
                ...
                if ((raw->blocks[i].rotation >= config_.min_angle
                    ...
                    if (pointInRange(distance)) {///여기서 정해준 거리 이내 에서 것 만 퍼블리쉬 되는것이다.
                        ...
                        velodyne_msgs::info pointinfo;
                        velodyne_msgs::cpoint cpoint;

                        pointinfo.hori = line_change;
                        pointinfo.ring = corrections.laser_ring;
                        cpoint.x = point.x;
                        cpoint.y = point.y;
                        cpoint.z = point.z;
                        cpoint.intensity = point.intensity;

//                        if(corrections.laser_ring > 31)
//                            std::cout<<pointinfo.ring<<std::endl;
                        my_msg->infos.push_back(pointinfo);
                        my_msg->cpoints.push_back(cpoint);
...

  {
    ...
    void RawData::unpack_vlp16(const velodyne_msgs::VelodynePacket &pkt,
                               VPointCloud &pc ,velodyne_msgs::custompoint *my_msg)
    {///16줄이 다 있을때 애기같은데?
        float azimuth;
        float azimuth_diff;
        ...
        const raw_packet_t *raw = (const raw_packet_t *) &pkt.data[0];    ///요기서 정보가 나오는 것 같은데.
        ...
        for (int block = 0; block < BLOCKS_PER_PACKET; block++) {
            ...
            for (int firing=0, k=0; firing < VLP16_FIRINGS_PER_BLOCK; firing++){
                for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){///여기서 16채널이 다 나오는 것 같은데. 일반적으로 정상적인게 16채널짜리이니까
                    velodyne_pointcloud::LaserCorrection &corrections =
                    ...
                    if ((azimuth_corrected >= config_.min_angle
                        ...
                        if (pointInRange(distance)) {///내가 정해준 사이즈 안에 성립할때
                            ...
                            pointinfo.hori = firing;
                            pointinfo.ring = corrections.laser_ring;
                            cpoint.x = point.x;
                            cpoint.y = point.y;
                            cpoint.z = point.z;
                            cpoint.intensity = point.intensity;
//                            std::cout<<pointinfo.hori<<std::endl;
                            my_msg->infos.push_back(pointinfo);
                            my_msg->cpoints.push_back(cpoint);
